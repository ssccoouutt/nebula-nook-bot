...
export const products = sqliteTable("products", { id: integer("id").primaryKey({ autoIncrement: true }), name: text("name").notNull(), description: text("description").notNull(), details: text("details").default("").notNull(), priceCents: integer("priceCents").notNull(), stock: integer("stock").default(0).notNull(), inventoryText: text("inventoryText").default("").notNull(), deliveryMode: text("deliveryMode").default("automatic").notNull(), warrantyDays: integer("warrantyDays").default(0).notNull(), imageUrl: text("imageUrl").default("").notNull(), active: integer("active").default(1).notNull(), freeEligible: integer("freeEligible").default(0).notNull(), freeWindowMs: integer("freeWindowMs"), ...timestamps });
...
export const orders = sqliteTable("orders", { id: integer("id").primaryKey({ autoIncrement: true }), botUserId: integer("botUserId").notNull(), productId: integer("productId").notNull(), kind: text("kind").notNull(), amountCents: integer("amountCents").notNull(), status: text("status").default("pending").notNull(), ...timestamps });
...
export const referrals = sqliteTable("referrals", { id: integer("id").primaryKey({ autoIncrement: true }), referrerId: integer("referrerId").notNull(), referredUserId: integer("referredUserId").notNull().unique(), bonusCents: integer("bonusCents").default(0).notNull(), createdAt: integer("createdAt", { mode: "timestamp_ms" }).notNull().defaultNow() });
