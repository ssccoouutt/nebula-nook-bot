...
export const products = sqliteTable("products", { id: integer("id").primaryKey({ autoIncrement: true }), name: text("name").notNull(), description: text("description").notNull(), priceCents: integer("priceCents").notNull(), stock: integer("stock").default(0).notNull(), active: integer("active").default(1).notNull(), freeEligible: integer("freeEligible").default(0).notNull(), freeWindowMs: integer("freeWindowMs"), ...timestamps });
