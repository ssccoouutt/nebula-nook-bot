...
export function parseBulkPricing(value: string | null | undefined): BulkPricingTier[] {
  ...
  const tiers = lines.map((line) => {
    const match = line.match(/^(\d+)(?:\s*-\s*(\d+)|\s*\+)\s*[:=]\s*\$?([0-9]+(?:\.[0-9]{1,2})?)$/);
...
export function resolveBulkUnitPriceCents(basePriceCents: number, bulkPricing: string | null | undefined, quantity: number) {
  ...
  const tier = parseBulkPricing(bulkPricing).find((item) => safeQuantity >= item.minQuantity && (item.maxQuantity === null || safeQuantity <= item.maxQuantity));
...
export function formatBulkPricingForUsers(bulkPricing: string | null | undefined, availableStock?: number) {
  return parseBulkPricing(bulkPricing).filter((tier) => availableStock === undefined || availableStock >= tier.minQuantity).map((tier) => `• ${tier.minQuantity}${tier.maxQuantity === null ? "+" : `-${tier.maxQuantity}`} codes → $${(tier.unitPriceCents / 100).toFixed(2)} each`).join("\n");
}
