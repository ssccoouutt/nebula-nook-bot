...
- [x] Trigger the Koyeb redeploy and verify the live asset hash and dashboard counts after startup.
...
- [x] Fix the open-ticket list showing #2 when the newly created ticket is reported as #4, so listing, creation notice, /reply, and /close all use one canonical ID.
- [x] Add a regression test for a newly created ticket appearing with its canonical ID in the admin open-ticket list.
...
- [x] Show total sold count in each product detail and add configurable product sorting by alphabetical or most-sold order in Telegram and dashboard.
- [x] Make BEP20 verification retry invalid hashes/errors until cancellation or another action, with an explicit cancel control for wallet and direct purchase flows.
- [x] Add dashboard user sorting by total amount spent.
