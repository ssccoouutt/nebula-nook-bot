...
2026-08-13 09:34 UTC: Koyeb service manager confirmed the public service is in Frankfurt, sourced from ssccoouutt/nebula-nook-bot main. Active deployment a557be77 was replaced by queued deployment f8133398 for commit a1ba39ac after a manual redeploy. The service has one free instance and zero attached volumes. The new build progressed through repository clone, Docker start, analysis, runtime detection, cache restore, and container build; deployment/scaling were not yet started at the last capture. Public URL: https://cognitive-quintilla-techzone3228-89a97258.koyeb.app/.

2026-08-13 09:35 UTC: Koyeb deployment f8133398 for commit a1ba39ac completed the build and entered Starting in Frankfurt. The public root now serves the Nebula Nook Bot operations console with title “Nebula Nook Bot”, confirming the corrected production static path. The health endpoint reports active=true and runtime=koyeb; pending Telegram count remains 3 with the prior 500 error, so that error is not yet cleared by a delivered update. Koyeb service manager still showed 0/1 during startup capture; continue until Healthy before interpreting webhook behavior.

2026-08-13 09:52 UTC: Koyeb deployment f8133398 is Healthy in Frankfurt with 1/1 running from commit a1ba39ac. Console startup logs show encrypted config loaded, OAuth disabled warning, server listening on localhost:8000, then `ELIFECYCLE Command failed with exit code 137`; the Koyeb event text says this commonly occurs when the instance runs out of memory (OOM), followed by automatic recovery and a later `Instance is healthy. All health checks are passing.` The build completed without the removed Manus Vite package in the dependency list. The active issue to fix is startup memory pressure/exit 137, not a missing DATABASE_URL exception.

Latest Koyeb console evidence: deployment ee13b65c is provisioning from commit 2ed3ef9a (webhook immediate-ack fix); the prior 354e5e85 deployment remains the active Healthy replica during cutover. The live health endpoint is reachable and still reports the historical Telegram 500 metadata with five pending updates; a harmless empty POST correctly returned 400 invalid Telegram update, confirming the webhook route is reachable.

2026-08-13 latest: Koyeb queued deployment 8ebce643 from commit bc6dd604 to deploy the SQLite parameter normalization fix; active deployment ee13b65c from 2ed3ef9a remains healthy during cutover. The active console log shows `ERR_INVALID_ARG_TYPE` from SQLite parameter binding during Telegram background processing. The new adapter and focused regression test are validated locally; live verification awaits cutover.
2026-08-13 current console capture: The authenticated Koyeb service overview rendered successfully. Active deployment `8ebce643` is Healthy, commit `bc6dd604`, Frankfurt, 1/1 running, zero attached volumes. Startup logs show server listening on port 8000, Telegram webhook configured for the Koyeb endpoint, health checks passing, and local SQLite at `/workspace/data/nebula-nook.sqlite`. The visible warnings are Node's experimental SQLite warning and an unused `OAUTH_SERVER_URL` warning; no new Telegram dispatch exception is visible in the captured log window. The console command input had previously been corrupted by duplicated keystrokes (`/bin/seecchhoo`), so that shell error is a console-input artifact, not an application failure.
