...
## Live Nebula Nook style verification

After the production checkpoint, Telegram Web showed the live Nebula Nook controls as ordinary `<button>` elements with the client-rendered class `Button HhayjUvb tiny primary has-ripple no-upper-case`. A direct DOM sample included visible `Shop` and `Buy now` controls with that class. The same sample showed the published Shop interaction and multiple `Buy now` controls; computed style inspection reported `background: rgba(74, 142, 58, 0.55)` and white text. Telegram Web normalizes both Bot API `primary` and `success` style values into its client-side `primary` class/color treatment in this session, so the Bot API semantic distinction is preserved in the payload even though the web client’s rendered appearance is the same green style.
