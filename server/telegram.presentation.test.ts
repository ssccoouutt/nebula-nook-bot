...
describe("Telegram presentation and notification helpers", () => {
  ...
  it("keeps core messages emoji-led and HTML formatted", () => {
    const home = formatHomeMessage({ firstName: "Rashid", username: "rashid", balanceCents: 1000, totalSpentCents: 2750, referrals: 3, access: true });
  ...
  it("keeps all requested group notifications anonymous", () => {
    ...
    for (const message of [reward, referral]) {
  ...
  it("routes every inline callback action deterministically", async () => {
    ...
    expect(parseTelegramCallbackAction("claim:7")).toBeNull();
  ...
  it("renders the Qamify-style quantity and confirmation steps", () => {
    ...
    expect(buildCustomQuantityKeyboard(7).inline_keyboard[0][0]).toMatchObject({ text: "✖️ Cancel", callback_data: "buycancel:7" });
  ...
  it("keeps Bot Statistics on the administrator keyboard instead of the normal-user menu", () => {
    ...
    expect(adminRows.flat().some((button) => ["shop", "wallet", "orders", "profile", "developer_api", "support"].includes(button.callback_data ?? ""))).toBe(false);
  });
  ...
  it("assigns Telegram primary and success styles to representative keyboards", () => {
    ...
    expect(compactShop[0][0]).toMatchObject({ callback_data: "product:1", style: "primary", text: "✨ Sample · $1.00 (3)" });
    ...
    expect(compactShop.at(-1)).toEqual(expect.arrayContaining([
      expect.objectContaining({ text: "🔄 Refresh", callback_data: "shop:0", style: "success" }),
      expect.objectContaining({ text: "🏠 Back to home", callback_data: "home", style: "success" }),
    ]));
    ...
    expect(shop[0][0]).toMatchObject({ callback_data: "product:7", style: "primary", text: "✨ Gemini Pro · $0.01 (1)" });
    expect(shop.flat().some((button) => button.callback_data === "shop:1")).toBe(false);
    expect(shop.at(-1)).toEqual(expect.arrayContaining([
      expect.objectContaining({ callback_data: "shop:0", style: "success" }),
      expect.objectContaining({ callback_data: "home", style: "success" }),
    ]));
  ...
  it("builds a Qamify-style masked purchase announcement with a product deep link", () => {
    ...
    expect(maskPurchaseName("𝖄oussef")).toBe("𝖄*****");
