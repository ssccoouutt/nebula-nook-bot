...
CREATE TABLE IF NOT EXISTS botUsers (id INTEGER PRIMARY KEY AUTOINCREMENT, telegramUserId INTEGER NOT NULL UNIQUE, username TEXT, firstName TEXT, lastName TEXT, referralCode TEXT NOT NULL UNIQUE, referredById INTEGER, balanceCents INTEGER NOT NULL DEFAULT 0, referralCredits INTEGER NOT NULL DEFAULT 0, accessGranted INTEGER NOT NULL DEFAULT 0, createdAt INTEGER NOT NULL DEFAULT (strftime('%s','now') * 1000), updatedAt INTEGER NOT NULL DEFAULT (strftime('%s','now') * 1000));
...
function ensureProductColumns(client: DatabaseClient) {
  ...
  const additions: Array<[string, string]> = [
    ...
    ["shopEligible", "INTEGER NOT NULL DEFAULT 1"],
    ["bulkPricing", "TEXT NOT NULL DEFAULT ''"],
