...
describe("Binance Pay transaction verification", () => {
  ...
  it("accepts a matching positive supported-asset receipt", async () => {
    ...
    const result = await findBinancePayTransaction("TX-123456", fetcher);
  ...
  it("rejects missing, unsupported, and non-positive receipts", async () => {
    ...
    expect(await findBinancePayTransaction("TX-123456", missing)).toMatchObject({ ok: false, reason: "not_found" });
    ...
    expect(await findBinancePayTransaction("TX-123456", unsupported)).toMatchObject({ ok: false, reason: "unsupported_asset" });
    ...
    expect(await findBinancePayTransaction("TX-123456", negative)).toMatchObject({ ok: false, reason: "not_received" });
  });
});
